...
// This hook is designed for Supabase but we're using Prisma
// For now, disable this to fix compilation
type DatabaseSchema = any // Database['public']
...
type SupabaseTableData<T extends SupabaseTableName> = any // DatabaseSchema['Tables'][T]['Row']

type SupabaseSelectBuilder<T extends SupabaseTableName> = any
// ReturnType<PostgrestQueryBuilder<DatabaseSchema, DatabaseSchema['Tables'][T], T>['select']>
...
interface UseSupabaseInfiniteProps<T extends SupabaseTableName, Query extends string = '*'> {
...
function createStore<TData extends SupabaseTableData<T>, T extends SupabaseTableName>(
  props: UseSupabaseInfiniteProps<T>
) {
  ...
  const refetch = async () => {
    setState({ isLoading: true, isSuccess: false, data: [], hasInitialFetch: false })
    await fetchNextPage()
...
>(props: UseSupabaseInfiniteProps<T>) {
  ...
  return {
    ...
    refetch: storeRef.current.refetch,
  }
}
