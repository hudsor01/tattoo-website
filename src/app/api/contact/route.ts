...
export async function POST(request: NextRequest) {
  try {
    ...
    if (formFields['hasFiles'] === 'true' && Object.keys(files).length > 0) {
      fileUrls = await saveFiles(files, contactSubmission.id);
    ...
    await sendEmail({
      to: (process.env['ADMIN_EMAIL'] || 'fennyg83@gmail.com') as string, // Default to the artist's email
      ...
      templateData: {
        ...
        referralSource: validatedData.referralSource || 'Not specified',
