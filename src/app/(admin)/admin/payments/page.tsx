...
import { Search, Filter, MoreHorizontal, RefreshCw, DollarSign, CreditCard, TrendingUp, TrendingDown } from 'lucide-react'
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats?.paymentsByStatus && stats.totalPayments > 0
                ? Math.round((stats.paymentsByStatus.find(s => s.status === PaymentStatus.COMPLETED)?._count.id || 0) / stats.totalPayments * 100)
                : 0}%
            </div>
          </CardContent>
        </Card>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {stats.paymentsByStatus.map((statusData) => (
                <div key={statusData.status} className="text-center">
                  <Badge className={statusColors[statusData.status]}>
                    {statusData.status}
                  </Badge>
                  <div className="mt-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedPayment.completedAt && (
                  <div>
                    ...
                    <p>{format(new Date(selectedPayment.completedAt), 'PPP p')}</p>
                  </div>
                )}
              </div>
