...
import type { VirtualizedBookingsListProps } from '@prisma/client';
...
export default function VirtualizedBookingsList({
  ...
  // Fetch appointments with tRPC (replacing getRecentBookings)
  const { data: bookingsData, isLoading } = trpc.appointments.getAll.useQuery({
    limit: 50,
  ...
  const filteredBookings = useMemo(() => {
    ...
    let filtered = bookingsData.items;

    // Search filter
    if (searchTerm) {
      const searchLower = searchTerm.toLowerCase();
      filtered = filtered.filter((booking) => {
        // Check each field, returning early if a match is found
        if (booking.clientName?.toLowerCase().includes(searchLower)) {
          return true;
        }
        
        if (booking.clientEmail?.toLowerCase().includes(searchLower)) {
          return true;
        }
        
        if (booking.tattooStyle?.toLowerCase().includes(searchLower)) {
          return true;
        }
        
        if (booking.description?.toLowerCase().includes(searchLower)) {
          return true;
        }
              ...
              <Card className="border-l-4 border-l-blue-500">
                <CollapsibleTrigger asChild>
                  <CardHeader
                    ...
                    onClick={() => {
                      toggleExpanded(parseInt(booking.id));
                    }}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <CardTitle className="text-base">{booking.clientName}</CardTitle>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <ChevronDown
                          className={`w-4 h-4 transition-transform ${
                            expandedBookings.has(parseInt(booking.id)) ? 'rotate-180' : ''
                          }`}
                        />
                      </div>
                    </div>
                  ...
                  <CardContent className="pt-0">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <div className="space-y-1 text-sm">
                          <p>
                            ...
                            {booking.tattooStyle ?? 'Not specified'}
                          </p>
                          <p>
                      ...
                      <div>
                        ...
                        <div className="space-y-1 text-sm">
                          <p>
                            <span className="text-gray-500">Email:</span> {booking.clientEmail}
                          </p>
                          <p>
