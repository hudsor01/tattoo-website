  ...
  useEffect(() => {
    const fetchDashboardData = async (): Promise<void> => {
      try {
        ...
        const customersThisMonth: number =
          customers.filter((c: Customer) => new Date(c.createdAt) >= thisMonth).length ?? 0;
        const customersLastMonth: number =
          customers.filter(
            (c: Customer) =>
        ...
        const completedBookings: number =
          calBookings.filter((b: Booking) => b.status === 'completed').length ?? 0;
        const upcomingBookings: number =
          calBookings.filter(
            (b: Booking) => b.status === 'scheduled' && new Date(b.start_time) > now
          ).length ?? 0;
        const totalBookings: number = calBookings.length ?? 0;

        const completionRate: number =
          totalBookings > 0 ? Math.round((completedBookings / totalBookings) * 100) : 0; // No fake data - show 0 if no bookings
